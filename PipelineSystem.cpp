...
void CalculateMaxFlow() {
	system("cls");
	std::cout << "=== Расчет максимального потока между КС ===" << std::endl;

	if (css.size() < 2) {
		std::cout << "Ошибка: Для расчета потока необходимо как минимум 2 КС!" << std::endl;
		std::cout << "Нажмите Enter для продолжения...";
		std::cin.ignore(1000, '\n');
		while (std::cin.get() != '\n');
		return;
	...
	while (!(std::cin >> sinkID) || css.find(sinkID) == css.end() || sinkID == sourceID) {
		if (sinkID == sourceID) {
			std::cout << "Ошибка! Источник и сток не могут совпадать. Введите другой ID: ";
		}
		else {
			std::cout << "Ошибка! Введите существующий ID КС: ";
		}
	...
	std::cout << "Источник: КС " << sourceID << " (" << css.find(sourceID)->second.getName() << ")" << std::endl;
	std::cout << "Сток: КС " << sinkID << " (" << css.find(sinkID)->second.getName() << ")" << std::endl;
	std::cout << "Максимальный поток: " << std::fixed << std::setprecision(2) << maxFlow << " м³/ч" << std::endl;

	if (maxFlow == 0.0) {
		std::cout << "Примечание: Поток равен 0. Возможные причины:" << std::endl;
		std::cout << "1. Нет пути между выбранными КС" << std::endl;
		std::cout << "2. Все трубы на пути находятся в ремонте" << std::endl;
		std::cout << "3. Сеть не содержит соединений между этими КС" << std::endl;
	}
...
void FindShortestPath() {
	system("cls");
	std::cout << "=== Поиск кратчайшего пути между КС ===" << std::endl;

	if (css.size() < 2) {
		std::cout << "Ошибка: Для поиска пути необходимо как минимум 2 КС!" << std::endl;
		std::cout << "Нажмите Enter для продолжения...";
		std::cin.ignore(1000, '\n');
		while (std::cin.get() != '\n');
		return;
	...
	while (!(std::cin >> endID) || css.find(endID) == css.end() || endID == startID) {
		if (endID == startID) {
			std::cout << "Ошибка! Начальная и конечная КС не могут совпадать. Введите другой ID: ";
		}
		else {
			std::cout << "Ошибка! Введите существующий ID КС: ";
		}
	...
	std::cout << "Начальная КС: " << startID << " (" << css.find(startID)->second.getName() << ")" << std::endl;
	std::cout << "Конечная КС: " << endID << " (" << css.find(endID)->second.getName() << ")" << std::endl;
	...
	if (distance == infinityValue) {
		...
		std::cout << "1. Нет соединения между выбранными КС" << std::endl;
		std::cout << "2. Все возможные пути содержат трубы в ремонте" << std::endl;
	}
	else {
		std::cout << "Длина кратчайшего пути: " << std::fixed << std::setprecision(2) << distance << " км" << std::endl;
		std::cout << "Маршрут: ";
					...
					if (conn.destinationCS == v) {
						...
						if (pipeIt != pipes.end()) {
							std::cout << "  КС " << u << " -> КС " << v
								...
								<< ", производительность: " << gasNetwork.getPipePerformance(conn.pipeID) << " м³/ч"
								<< ", статус: " << (pipeIt->second.getStatus() ? "в ремонте" : "работает")
								<< std::endl;
						}
						break;
					}
